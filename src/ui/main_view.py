...
class MainView():
    ...
    def add_analysis_buttons(self):
        ...
        self.generate_primers_button = Button(
            self.input_frame, text="Generate sequencing primers", command=self.generate_primers)
    ...
    def add_users_dna_fragments_dropdown_list(self):
        ...
        for dna_fragment in self.dna_fragment_service.get_all_dna_fragments_by_owner_name(self.account_service.logged_in_user.name):
    ...
    def add_notification(self, notification, labels):
        if not labels[:-1]:
            ...
            labels[-1].config(text=labels[-2].cget("text"),
                              fg=labels[-2].cget("fg"))
    ...
    def add_new_dna_fragment(self):
        if self.help_activated == True:
            ...
            self.add_notification(
                ["If there are any problems in adding the DNA fragment, you will be notified of it here.", "white"], self.notification_labels)
            self.add_notification(
                ["The added DNA fragments will be stored in a database on your PC hard disk so you can use them also later.", "white"], self.notification_labels)
            self.add_notification(
                ["You cannot add two DNA fragments with the same name. Sequence can contain only natural nucleotides A, T, G and C.", "white"], self.notification_labels)
            self.add_notification(
                ["You can add new DNA fragments here. The button will be activated when you have entered name and sequence.", "white"], self.notification_labels)
        else:
            notification = self.dna_fragment_service.try_to_create_new_dna_fragment_and_return_notification(
                Entry.get(self.name_input), Entry.get(self.sequence_input), self.account_service.logged_in_user.name)
    ...
    def translate(self):
        if self.help_activated == True:
            self.add_notification(["", "white"], self.notification_labels)
            self.add_notification(
                ["If there are any problems in generating the protein sequence, you will get notified of it here.", "white"], self.notification_labels)
            self.add_notification(
                ["The protein sequence will be saved to your working directory in 'Proteins' folder.", "white"], self.notification_labels)
            self.add_notification(
                ["You might find this functionality useful if you want to see how mutations affect the protein sequence.", "white"], self.notification_labels)
            self.add_notification(
                ["Translate button is used to generate the protein sequence the DNA fragment codes for.", "white"], self.notification_labels)
        else:
            notification = self.protein_service.attempt_translation_and_return_notification(
                self.dna_fragment_service.get_dna_fragment_by_name_and_owner_name(self.dna_fragment_name.get(), self.account_service.logged_in_user.name), self.account_service.logged_in_user.directory)
            self.add_notification(notification, self.notification_labels)
    ...
    def generate_primers(self):
        if self.help_activated == True:
            self.add_notification(["", "white"], self.notification_labels)
            self.add_notification(
                ["If there are any problems in generating the primers, you will get notified of it here.", "white"], self.notification_labels)
            self.add_notification(
                ["The primers will be saved in your working directory in the 'primers' folder in an Excel file.", "white"], self.notification_labels)
            self.add_notification(
                ["The primers will be by the name of the DNA fragment, the strand (forward or reverse) and number of the primer.", "white"], self.notification_labels)
            self.add_notification(
                ["You will be able to change these values by clicking 'Settings' from the menu.", "white"], self.notification_labels)
            self.add_notification(
                ["This is done by searching sequences in the strands that have certain length and GC content.", "white"], self.notification_labels)
            self.add_notification(
                ["Generate sequencing primers button is used to generate sequencing primers for both strands of the DNA fragment.", "white"], self.notification_labels)
        else:
            self.primer_service = PrimerService(
                self.account_service.logged_in_user.directory, self.account_service.logged_in_user.primer_length, self.account_service.logged_in_user.primer_gc_content)
            notification = self.primer_service.attempt_primer_generation_and_return_notification(
                self.dna_fragment_service.get_dna_fragment_by_name_and_owner_name(self.dna_fragment_name.get(), self.account_service.logged_in_user.name))
            self.add_notification(notification, self.notification_labels)
    ...
    def select_dna_fragment_help(self):
        if self.help_activated == True:
            self.add_notification(["", "white"], self.notification_labels)
            self.add_notification(
                ["You can select DNA fragments that you have added previosly for analysis here.", "white"], self.notification_labels)
    ...
    def help(self):
        if self.help_activated == False:
            ...
            self.add_notification(["", "white"], self.notification_labels)
            self.add_notification(
                ["To exit help mode, please click 'Help' from the menu again.", "white"], self.notification_labels)
            self.add_notification(
                ["Welcome to help mode! Please click a button to see instructions how to see what it is used for.", "white"], self.notification_labels)
        else:
            self.add_notification(["", "white"], self.notification_labels)
            self.add_notification(
                ["Excellent, now let's get those sequencing primers designed!", "white"], self.notification_labels)
            self.help_string = "Help mode activated!"
    ...
    def settings(self):
        if self.help_activated == True:
            self.add_notification(["", "white"], self.notification_labels)
            self.add_notification(
                ["Opens settings where you can change working directory, sequencing primer length and GC content, or delete your account.", "white"], self.notification_labels)
        else:
    ...
    def exit(self):
        if self.help_activated == True:
            self.add_notification(["", "white"], self.notification_labels)
            self.add_notification(
                ["Click here to exit the application.", "white"], self.notification_labels)
        else:
            self.root.destroy()
