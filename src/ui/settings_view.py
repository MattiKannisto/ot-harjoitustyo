...
class SettingsView:
    ...
    def active(self):
        ...
        if not self.frame:
            ...
            primer_length_instructions = Label(
            ...
            self.primer_length.set(
                str(self.account_service.logged_in_user.primer_length))
            primer_length_dropdown_menu = OptionMenu(
                self.frame, self.primer_length, *PRIMER_LENGTH_RANGE)
            ...
            self.gc_content.set(
                str(int(self.account_service.logged_in_user.primer_gc_content*100)))
            gc_content_dropdown_menu = OptionMenu(
                self.frame, self.gc_content, *PRIMER_GC_CONTENT_RANGE)
    ...
    def _save_settings_and_exit(self):
        self.account_service.logged_in_user.primer_length = int(
            self.primer_length.get())
        self.account_service.logged_in_user.primer_gc_content = float(
    ...
    def _change_working_directory(self):
        if self.account_service.logged_in_user:
            directory = filedialog.askdirectory(
                initialdir=self.account_service.logged_in_user.directory)
            if directory:
